...
namespace FSEngine
{
    ...
    public class Noise
    {
        ...
        private double interpolation_factor = 100;

        public Noise(double persistence, double frequency, double amplitude, int octaves, int randomSeed)
        {
            this.persistence = persistence;
            this.frequency = frequency;
            this.freq_to = frequency;
            this.amplitude = amplitude;
            this.amp_to = amplitude;
            this.octaves = octaves;
            this.randomSeed = randomSeed;
        }
        public void SetInterpolationFactor(double interpolation)
        {
            interpolation_factor = interpolation;
        }

        public void ToAmplitude(double amp)
        {
            amp_to = amp;
        }
        public void ToAmplitude(double amp, double interpolation_factor)
        {
            amp_to = amp;
            this.interpolation_factor = interpolation_factor;
        }
        public void ToFrequency(double freq)
        {
            freq_to = freq;
        }
        public void ToFrequency(double freq, double interpolation_factor)
        {
            freq_to = freq;
            this.interpolation_factor = interpolation_factor;
        }
        ...
        {
            amplitude += (amp_to - amplitude) / interpolation_factor;
            frequency += (freq_to - frequency) / interpolation_factor;
        }
        public double Get1DInterpolated(double x)
        {
            Interpolate();
            return Get2D(x, 0);
        }
        public double Get2DInterpolated(double x, double y)
        {
            Interpolate();
            return Get2D(x, y);
        }
        public double Get3DInterpolated(double x, double y, double z)
        {
            Interpolate();
            return Get3D(x, y, z);
        }
        ...
        /// GET \\\
        /// *** \\\
        public double GetWorm2D(double x, double y, double ridge, double range)
        {
            if (range > ridge)
            {
                double r = range;
            ...
            double worm = Get2D(x, y);
            if (worm > ridge - range && worm < ridge + range)
                return 1;
            else
                return 0;
        }
